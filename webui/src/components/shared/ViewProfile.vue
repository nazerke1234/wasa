<template>
  <div class="profile-container">
    <div class="profile-card">
      <h2>Your Profile</h2>

      <!-- Profile photo -->
      <div class="profile-photo">
        <img :src="user.photo || defaultPhoto" alt="Profile Photo" />
        <button @click="enablePhotoEdit" v-if="!isEditingPhoto">Change</button>
        <input 
          v-if="isEditingPhoto" 
          v-model="newPhotoUrl" 
          type="text" 
          placeholder="Enter new photo URL"
        />
        <button v-if="isEditingPhoto" @click="saveProfilePhoto">Save</button>
      </div>

      <!-- Username -->
      <div class="profile-info">
        <p><strong>Username:</strong> {{ user.username }}</p>
        <button @click="enableUsernameEdit" v-if="!isEditingUsername">Edit</button>
        <input v-if="isEditingUsername" v-model="newUsername" type="text" placeholder="New username" />
        <button v-if="isEditingUsername" @click="saveUsername">Save</button>
      </div>

      <p v-if="isLoading" class="loading">Updating...</p>
    </div>
  </div>
</template>

<script>
import axios from 'axios';

export default {
  name: 'ViewProfile',
  data() {
    return {
      user: {
        username: '',
        photo: ''
      },
      isEditingUsername: false,
      newUsername: '',
      isEditingPhoto: false,
      newPhotoUrl: '',
      isLoading: false,
      defaultPhoto: 'data:image/jpeg;base64,' // Запасное фото
    };
  },
  mounted() {
    this.getUserProfile();
  },
  methods: {
    getUserProfile() {
      const token = localStorage.getItem('access_token');
      axios.get('http://localhost:8080/api/me', {
        headers: { Authorization: `Bearer ${token}` }
      })
      .then((response) => {
        this.user = response.data;
        this.newUsername = this.user.username;
      })
      .catch(error => console.error('Error fetching user profile:', error));
    },

    enableUsernameEdit() {
      this.isEditingUsername = true;
    },

    saveUsername() {
      if (!this.newUsername.trim()) {
        alert('Username cannot be empty');
        return;
      }
      this.isLoading = true;
      const token = localStorage.getItem('access_token');
      axios.put('http://localhost:8080/api/me/username', {
        new_username: this.newUsername
      }, {
        headers: { Authorization: `Bearer ${token}` }
      })
      .then(() => {
        alert('Username updated successfully');
        this.user.username = this.newUsername;
        this.isEditingUsername = false;
        this.isLoading = false;
      })
      .catch(error => {
        console.error('Error updating username:', error);
        this.isLoading = false;
      });
    },

    enablePhotoEdit() {
      this.isEditingPhoto = true;
      this.newPhotoUrl = this.user.photo || this.defaultPhoto;
    },

    saveProfilePhoto() {
      if (!this.newPhotoUrl.trim()) {
        alert('Photo URL cannot be empty');
        return;
      }
      this.isLoading = true;
      const token = localStorage.getItem('access_token');
      axios.put('http://localhost:8080/api/me/photo', {
        photo_url: this.newPhotoUrl
      }, {
        headers: { Authorization: `Bearer ${token}` }
      })
      .then(() => {
        alert('Profile photo updated successfully');
        this.user.photo = this.newPhotoUrl;
        this.isEditingPhoto = false;
        this.isLoading = false;
      })
      .catch(error => {
        console.error('Error updating profile photo:', error);
        this.isLoading = false;
      });
    }
  }
};
</script>

<style scoped>
@import url('https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600&display=swap');

.profile-container {
  display: flex;
  justify-content: center;
  align-items: center;
  height: 100vh;
  background: linear-gradient(135deg, #1e3c3c, #2a5d5d);
  font-family: 'Poppins', sans-serif;
}

.profile-card {
  background: white;
  padding: 30px;
  border-radius: 15px;
  box-shadow: 0 4px 15px rgba(0, 0, 0, 0.2);
  text-align: center;
  width: 350px;
}

.profile-photo {
  position: relative;
  margin-bottom: 15px;
}

.profile-photo img {
  width: 120px;
  height: 120px;
  border-radius: 50%;
  border: 4px solid #2a5d5d;
  object-fit: cover;
}

.profile-photo button {
  margin-top: 10px;
  background: linear-gradient(45deg, #2a5d5d, #1e3c3c);
  color: white;
  border: none;
  padding: 8px 15px;
  border-radius: 5px;
  cursor: pointer;
  font-size: 14px;
  transition: 0.3s;
}

.profile-photo button:hover {
  background: linear-gradient(45deg, #1e3c3c, #0f2424);
}

.profile-photo input {
  width: 90%;
  padding: 8px;
  margin-top: 10px;
  border: 1px solid #ccc;
  border-radius: 5px;
}

.profile-info {
  margin-bottom: 15px;
}

.profile-info button {
  background: linear-gradient(45deg, #2a5d5d, #1e3c3c);
  color: white;
  border: none;
  padding: 8px 15px;
  border-radius: 5px;
  cursor: pointer;
  font-size: 14px;
  transition: 0.3s;
}

.profile-info button:hover {
  background: linear-gradient(45deg, #1e3c3c, #0f2424);
}

button {
  transition: 0.3s;
  font-weight: 500;
}

input {
  padding: 10px;
  width: 100%;
  border-radius: 5px;
  border: 1px solid #ccc;
  margin-top: 10px;
  font-size: 14px;
  font-family: 'Poppins', sans-serif;
}

.loading {
  color: #e67e22;
  font-weight: bold;
}
</style>
